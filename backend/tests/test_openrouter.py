...
async def test_strips_multiple_think_blocks(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
...
async def test_system_prompt_prepended(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    with patch("app.providers.openrouter.AsyncOpenAI", return_value=mock_client):
        ...
        await call_openrouter(
            "model", "Be helpful", [{"role": "user", "content": "Hello"}], "key"
        )
...
async def test_enable_thinking_passes_reasoning_param(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
...
async def test_reasoning_field_returned_as_thinking(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    """When the response message has a `reasoning` attribute (OpenRouter reasoning tokens), it's returned as thinking."""
    monkeypatch.setattr("app.providers.openrouter.LOGS_DIR", tmp_path)
    mock_client = make_mock_client("The answer")
    mock_client.chat.completions.create.return_value.choices[
        0
    ].message.reasoning = "Step-by-step reasoning"
    with patch("app.providers.openrouter.AsyncOpenAI", return_value=mock_client):
        from app.providers.openrouter import call_openrouter

        content, thinking = await call_openrouter(
            "model", "sys", [], "key", enable_thinking=True
        )
    assert content == "The answer"
    assert thinking == "Step-by-step reasoning"


async def test_disable_thinking_omits_reasoning_param(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
