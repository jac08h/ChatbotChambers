...
def test_full_conversation_flow(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "app.engine.call_litellm",
        AsyncMock(side_effect=[("Hello!", ""), ("/leave", "")]),
    )

    client = TestClient(app)
    with client.websocket_connect("/ws") as ws:
        ws.send_json({"type": "start", "config": LITELLM_CONFIG})
...
def test_done_reason_leave(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("app.engine.call_litellm", AsyncMock(return_value=("/leave", "")))
...
def test_stop_command_accepted_without_error(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("app.engine.call_litellm", AsyncMock(return_value=("Hello!", "")))
...
def test_missing_start_message_returns_error():
